...
class TestAliens:
    def test_aliens_task(self, browser, links):
        ...
        link = links
        browser.get(link)
        browser.implicitly_wait(60)  # ждём прогрузки страницы (неявное ожидание)
        ...
        input_email.send_keys(auth[0])

        input_password = browser.find_element(By.ID, "id_login_password")
        input_password.send_keys(auth[1])  # вводим пароль

        button_login = browser.find_element(By.CSS_SELECTOR, ".sign-form__btn.button_with-loader")  # нажимаем на кнопку "Войти"
        button_login.click()
        time.sleep(10)
        ...
        try:
            ...
            assert answer_feedback.text == "Correct!", f"Wrong answer! {answer_feedback.text}"