...
def test_button_start():
    ...
    link_6_6_11 = 'https://parsinger.ru/methods/3/index.html'
    link_6_6_12 = 'https://parsinger.ru/selenium/5.6/1/index.html'

    try:
        options = webdriver.ChromeOptions()
        options.add_argument('--headless=new')
        browser = webdriver.Chrome(options=options)
        browser.get(link_6_6_12)
        ...
        for cookie in cookies:
            ...
            browser.refresh()
            if browser.find_element(By.ID, 'age').text != None:
                age = int(browser.find_element(By.ID, 'age').text.split(' ')[1])
            else:
                continue
            skils = int(len(browser.find_element(By.ID, 'skillsList').find_elements(By.TAG_NAME, 'li')))
            if age <= min_age and skils > max_skils:
                    min_age = age
                    max_skils = skils
                    result = browser.find_element(By.ID, 'cookie-message').text.split(' ')[1]
        print(result)
